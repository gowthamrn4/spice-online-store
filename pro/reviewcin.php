...
$row=mysql_fetch_array($select);
 ...
 ?>
        ...
        <ul>
            ...
            <li><a href="cinnamon.php"class="current">Cinnamon</a></li>
            </ul>
    </div>
...
echo $row['image']; ?>" alt="User profile picture" width="100" height="100" style="inline-box-align:initial"></div>
     	<div class="col_w420 float_l">
                        <?php
   $select=mysql_query("select * from cinnamon where number='".$_GET['edit']."'");
   $view=mysql_fetch_array($select);
?>
...
<div class="button"><a href="buy.php?edit=<?php echo mysql_result($select,$i,"number");?>">Buy</a></div>
 ...
 <table width="500" height="141">
  ...
  <tr>   <?php
   $select=mysql_query("select * from reviewcin where number='".$_GET['edit']."'");
   $view=mysql_fetch_array($select);
...
if($button)
 {
$insert=mysql_query("insert into reviewcin(no,name,image,time,review,mark,status)values('$no','$name','$image','$time','$review','$mark','$status')");
}
if($insert)
{
?>
