...
?>
  ...
  <?php
   $select=mysql_query("select * from clove where number='".$_GET['edit']."'");
...
$row=mysql_fetch_array($select);
 ...
 ?>
        ...
        <ul>
            ...
            <li><a href="garlic.php"class="current">Garlic</a></li>
...
echo $row['image']; ?>" alt="User profile picture" width="100" height="100" style="inline-box-align:initial"></div>
     	<div class="col_w420 float_l">
                        <?php
   $select=mysql_query("select * from garlic where number='".$_GET['edit']."'");
   $view=mysql_fetch_array($select);
?>
...
<span style="font-size:24px"><?php echo $view['name'];?></span></div>  <img src="images/0004.png" width="120" height="30"/>
...
<div class="button"><a href="buy.php?edit=<?php echo mysql_result($select,$i,"number");?>">Buy</a></div>
 ...
 <table width="500" height="141">
  ...
  <tr>   <?php
   $select=mysql_query("select * from reviewgarlic where number='".$_GET['edit']."'");
   $view=mysql_fetch_array($select);
...
if($button)
 {
$insert=mysql_query("insert into reviewga(no,name,image,time,review,mark,status)values('$no','$name','$image','$time','$review','$mark','$status')");
}
if($insert)
{
?>
